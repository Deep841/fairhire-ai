...
import { ArrowRight, Briefcase, CheckCircle2, Upload, Trophy, ScanText, KanbanSquare, Scale, BellRing, Send, MessageSquare, FilePlus, FileUp, UserCheck } from "lucide-react";
...
export default function Landing() {
  return (
    <div className="min-h-screen" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* Navbar */}
      <nav className="sticky top-0 z-50 glass-navbar px-6 h-16 flex items-center justify-between max-w-full">
        <div className="max-w-6xl mx-auto w-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/login" className="btn-glass-dark inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold">
      ...
      <section className="relative pt-24 pb-28 px-6 text-center overflow-hidden">
        <div className="relative max-w-4xl mx-auto">
          ...
          <p className="mt-6 text-lg text-slate-500 leading-relaxed max-w-2xl mx-auto">
            QuantumLogic Labs automates resume screening, AI-powered candidate scoring, hiring pipeline management, and interview scheduling so your HR team focuses on people, not paperwork.
          </p>
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/login" className="btn-glass-dark inline-flex items-center gap-2 px-7 py-3.5 rounded-2xl text-base font-semibold">
              Start hiring free <ArrowRight className="h-4 w-4" />
            </Link>
            <a href="#how-it-works" className="btn-glass inline-flex items-center gap-2 px-7 py-3.5 rounded-2xl text-base font-semibold">
              See how it works
            </a>
          </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900">Everything your HR team needs</h2>
            <p className="mt-3 text-base text-slate-500 max-w-xl mx-auto">From job posting to offer letter — one platform, zero spreadsheets.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="glass rounded-2xl p-6 hover:shadow-md transition-all">
                <div className="inline-flex p-2.5 rounded-xl mb-4" style={{ background: "rgba(0,0,0,0.05)" }}>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {STEPS.map(({ num, icon: Icon, title, desc }) => (
              <div key={num} className="glass rounded-2xl p-8 text-center relative">
                <span className="absolute top-4 right-5 text-xs font-bold text-slate-300">{num}</span>
                <div className="inline-flex p-4 rounded-2xl mb-5" style={{ background: "rgba(0,0,0,0.05)" }}>
      ...
      <section className="py-20 px-6">
        <div className="max-w-3xl mx-auto">
          <div className="glass rounded-3xl p-14 text-center">
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-4">Ready to transform your hiring?</h2>
            ...
            <Link to="/login" className="btn-glass-dark inline-flex items-center gap-2 px-8 py-4 rounded-2xl text-base font-bold">
      ...
      <footer className="py-10 px-6 border-t border-slate-100">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-slate-400">© {new Date().getFullYear()} QuantumLogic Labs. Built for modern HR teams.</p>
          <div className="flex items-center gap-4">
            <Link to="/login" className="text-xs text-slate-400 hover:text-slate-700 transition-colors">Sign in</Link>
            <Link to="/login" className="text-xs text-slate-400 hover:text-slate-700 transition-colors">Register</Link>
          </div>
        </div>
      </footer>
