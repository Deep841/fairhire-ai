...
export default function Jobs() {
  ...
  return (
    <div className="px-4 sm:px-0 space-y-8 max-w-4xl mx-auto">
      <div className="glass rounded-2xl shadow-card p-6 sm:p-8">
      ...
      {showForm && (
        <div className="glass rounded-2xl shadow-card p-6 sm:p-8">
          <h2 className="text-lg font-semibold text-white mb-5">Create job requisition</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            {error && (
              <div className="p-3 rounded-xl bg-red-500/20 border border-red-500/30 text-sm text-red-300">{error}</div>
            )}
        ...
        <div className="glass rounded-2xl shadow-card overflow-hidden">
          <ul className="divide-y divide-white/5">
            {jobs.map((job) => {
              ...
              return (
                <li key={job.id} className={`px-6 py-5 transition-colors ${
                  ...
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className={`text-sm font-semibold truncate ${
                          ...
                          {job.title}
                          {isActive && (
                            <span className="ml-2 text-xs font-medium text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded-full">
                              Active
                            </span>
                          )}
                        </p>
                        {firstLine && (
                          <p className="text-xs text-slate-500 truncate mt-0.5">{firstLine.slice(0, 100)}</p>
                        )}
                        {published.length > 0 && (
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                      ...
                      <Link to="/process-resumes" onClick={() => handleSelect(job.id)}
                        ...
                        Process resumes <ArrowRight className="h-3.5 w-3.5" />
                      </Link>
                    </div>
                  </div>
